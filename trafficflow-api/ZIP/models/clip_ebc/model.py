...
class CLIP_EBC(nn.Module):
    ...
    def _build_head(self) -> None:
        ...
        if self.zero_inflated:
            ...
            self.pi_heads = nn.ModuleList([conv1x1(in_channels, out_channels, bias=False) for _ in range(self.num_classes)])
            self.lambda_heads = nn.ModuleList([conv1x1(in_channels, out_channels, bias=False) for _ in range(self.num_classes)])

        else:
            self.logit_scale = nn.Parameter(torch.ones([]) * np.log(1 / 0.07), requires_grad=True)
            self.heads = nn.ModuleList([conv1x1(in_channels, out_channels, bias=False) for _ in range(self.num_classes)])
    ...
    def forward(self, image: Tensor):
        ...
        if self.zero_inflated:
            if self.training:
                all_pi_logit_maps = []
                all_lambda_logit_maps = []
                all_lambda_maps = []

            for i in range(self.num_classes):
                pi_image_feats, lambda_image_feats = self.pi_heads[i](image_feats), self.lambda_heads[i](image_feats)
                ...
                all_den_maps.append(den_map)
                if self.training:
                    all_pi_logit_maps.append(pi_logit_map)
                    all_lambda_logit_maps.append(lambda_logit_map)
                    all_lambda_maps.append(lambda_map)
            
            final_den_map = torch.cat(all_den_maps, dim=1)
            if self.training:
                return (
                    torch.stack(all_pi_logit_maps, dim=1), 
                    torch.stack(all_lambda_logit_maps, dim=1), 
                    torch.stack(all_lambda_maps, dim=1), 
                    final_den_map
                )
            else:
                return final_den_map
            ...
            for i in range(self.num_classes):
                curr_image_feats = self.heads[i](image_feats)
                curr_image_feats = F.normalize(curr_image_feats.permute(0, 2, 3, 1), p=2, dim=-1)

                text_feats = self.text_feats
                logit_scale = self.logit_scale.exp()

                logit_map = logit_scale * curr_image_feats @ text_feats.t()  # (B, H, W, N), logits per image
                ...
                if self.training:
                    all_logit_maps.append(logit_map)

            final_den_map = torch.cat(all_den_maps, dim=1)
            if self.training:
                return torch.stack(all_logit_maps, dim=1), final_den_map
            else:
                return final_den_map
