...
class Crowd(Dataset):
    ...
    def __check_sanity__(self) -> None:
        known_datasets = ["sha", "shb", "nwpu", "qnrf"]
        if self.dataset in known_datasets:
            if self.dataset == "sha":
                if self.split == "train":
            ...
            print(f"Using custom dataset '{self.dataset}'. Skipping sanity checks.")
            if self.split in ["train", "val"]:
                print(f"Found {len(self.image_names)} images and {len(self.label_names)} labels for the '{self.split}' split.")
    ...
    def __getitem__(self, idx: int) -> Union[Tuple[Tensor, Tensor, Tensor], Tuple[Tensor, Tensor, Tensor, str]]:
        ...
        label = torch.from_numpy(np.load(label_path, allow_pickle=True)).float()
        ...
        density_maps = torch.stack([generate_density_map(label, image.shape[-2], image.shape[-1], sigma=self.sigma, num_classes=self.num_classes) for image, label in zip(images, labels)], 0)
