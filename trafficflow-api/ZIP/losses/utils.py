...
def _reshape_density(density: Tensor, block_size: int) -> Tensor:
    assert len(density.shape) == 4, f"Expected 4D (B, C, H, W) tensor, got {density.shape}"
    C = density.shape[1]
    ...
    return density.reshape(density.shape[0], C, density.shape[2] // block_size, block_size, density.shape[3] // block_size, block_size).sum(dim=(-1, -3))
...
def _bin_count(density_map: Tensor, bins: List[Tuple[int, int]]) -> Tensor:
    ...
    for idx, (low, high) in enumerate(bins):
        ...
        class_map[mask] = idx
    return class_map.squeeze(1) if density_map.shape[1] == 1 else class_map
