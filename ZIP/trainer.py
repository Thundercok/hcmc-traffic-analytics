...
parser.add_argument("--num_classes", type=int, default=1, help="The number of classes for counting.")
...
parser.add_argument("--pretrained_weights", type=str, default=None, help="Path to a .pth file with model weights to load for fine-tuning.")
...
def run(local_rank: int, nprocs: int, args: ArgumentParser) -> None:
    ...
    if args.pretrained_weights:
        if os.path.exists(args.pretrained_weights):
            ...
            # The weights are nested in the dictionary, common for this project's checkpoints.
            if 'model_info' in state_dict and 'weights' in state_dict['model_info']:
                weights = state_dict['model_info']['weights']
            else:
                # Fallback for simpler .pth files that are just the state_dict
                weights = state_dict
            
            # Adjust keys for DDP-trained models if necessary
            ...
            if local_rank == 0:
                print(f"Warning: Pretrained weights path not found: {args.pretrained_weights}")
