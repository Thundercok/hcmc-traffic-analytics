...
class EBC(nn.Module):
    ...
    def _build_head(self) -> None:
        ...
        self.bin_heads = nn.ModuleList() # important
        if self.zero_inflated:
            self.pi_heads = nn.ModuleList()

        for _ in range(self.num_classes):
            if self.zero_inflated:
                self.bin_heads.append(conv1x1(
                    in_channels=channels,
                    out_channels=len(self.bins) - 1,
                ))
                self.pi_heads.append(conv1x1(
                    in_channels=channels,
                    out_channels=2,
    ...
    def forward(self, x: Tensor) -> Union[Tensor, Tuple[Tensor, Tensor]]:
        ...
        for i in range(self.num_classes):
            if self.zero_inflated:
                logit_pi_maps = self.pi_heads[i](x)  # shape: (B, 2, H, W)
                logit_maps = self.bin_heads[i](x)  # shape: (B, C, H, W)
                ...
                if self.training:
                    all_logit_pi_maps.append(logit_pi_maps)
                    all_logit_maps.append(logit_maps)
                    all_lambda_maps.append(lambda_maps)
                
                all_den_maps.append(den_maps)
            else:
                logit_maps = self.bin_heads[i](x)
        ...
        # Stack outputs: (B, Num_Classes, ...)
        final_den_maps = torch.cat(all_den_maps, dim=1) # (B, Num_Classes, H, W)

        if self.training:
            if self.zero_inflated:
                return (
                    torch.stack(all_logit_pi_maps, dim=1), # (B, Num_Classes, 2, H, W)
                    torch.stack(all_logit_maps, dim=1),    # (B, Num_Classes, C_bins, H, W)
                    torch.stack(all_lambda_maps, dim=1),   # (B, Num_Classes, 1, H, W)
                    final_den_maps
                )
            else:
                return (
                    torch.stack(all_logit_maps, dim=1), 
                    final_den_maps
                )
        else:
            return final_den_maps
