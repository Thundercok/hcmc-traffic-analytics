...
def update_eval_result(
    ...
    state_dict = model_info["weights"]

    for k, v in curr_scores.items():
        if k not in hist_scores:
            hist_scores[k] = []
        hist_scores[k].append(v)
        writer.add_scalar(f"val/{k}", v, epoch)

        if k not in best_scores:
            # Try to infer save_best_k from other keys, or default to 5
            save_best_k = 5
            if len(best_scores) > 0:
                save_best_k = len(list(best_scores.values())[0])
            best_scores[k] = [torch.inf] * save_best_k
        ...
        if loc is not None: 
            ...
            for i in range(len(best_scores[k]) - 1, loc, -1):
                if os.path.exists(os.path.join(ckpt_dir, f"best_{k}_{i-1}.pth")):
                    os.replace(os.path.join(ckpt_dir, f"best_{k}_{i-1}.pth"), os.path.join(ckpt_dir, f"best_{k}_{i}.pth"))
